...
def generar_pdf_despacho(data: PDFDespachoRequest):
    fecha = data.fecha.strftime("%Y-%m-%d")
    logger.info(f"➡ Fecha recibida: {fecha}")
    try:
        with ConexionSQL() as conn:
            cursor = conn.cursor
            logger.info(f"📦 Ejecutando SP: LISTADO_DOC_ACTA_DESP_VENTA '{fecha}'")
            cursor.execute("EXEC LISTADO_DOC_ACTA_DESP_VENTA ?", fecha)
        ...
        for docentry in docentries:
            ...
            for item in productos:
                ...
                productos_formateados.append({
                    ...
                    "condicion": safe(item.get("CodAlm")),
                })
            ...
            try:
                ruta = generar_pdf_acta_despacho(data_pdf)
        ...
        logger.critical(f"🔥 Error en generar_pdf_despacho: {e}")
        logger.debug(traceback.format_exc())
